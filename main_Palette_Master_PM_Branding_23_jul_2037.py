...
#        self.setWindowIcon(QIcon(get_data_path("gimp.ico")))  # Stubbed WEN logo path
        ...
        icon_path = self.get_icon_path("WEN_flow_Logo.ico")
#        icon_path = RESOURCES / "moon.ico"  # Use paths module
        print(f"[DEBUG] Searching at: {icon_path}")
...
#            print(f"Processing cell: slot_id={slot_id}, type={cell_type}")
            ...
            if cell_type == "grid" and slot_id.startswith("Button_"):
                ...
                icon_path = self.getr_icon_path("moon.ico")
                print(f"[DEBUG] Theme_Toggle searching at: {icon_path}")
                icon = QIcon(icon_path)

                self.theme_toggle_btn.setIcon(QIcon(self.get_icon_path("moon")))
...
# Define the method inside WENPalette
    ...
    def toggle_theme_mode(self):
        ...
        icon_path = self.theme_mgr.get_icon_path("WEN_flow_Logo.ico")
...
if __name__ == "__main__":
    ...
    window.set_branding_icon()  # <- theme_mgr is now safely available
    theme_mgr.load_theme("dark")

    window.show()
    print("[WEN] Window should now be visible.")
